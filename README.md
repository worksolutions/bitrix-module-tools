...
В осносном упрощение интерфейса заключается в передаче второго параметра ```CEvent::Send```, часто происходит ошибочная ситуация
...
местоположение отправки сообщения (код в обрпботчике).  Модули коробки ```Битрикса``` справляются так:
...
```WS-Tools``` упрощает использование и лишние действия разработчика. Возможно дальнейшее расширение функционала отправки почты.

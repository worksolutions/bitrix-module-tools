...
* `abort()` прерывание записи кэширования, данные записи возвращяются в поток
* `save($output = true)` сохранние записи кэширования в память, `$output` - параметр указывающий на необходимость направления данных в поток вывода, по умолчанию распечатывается
